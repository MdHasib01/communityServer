 ...
 */
router.get("/scrape-all", asyncHandler(async (req, res) => {
  try {
    const results = await scraperManager.scrapeAllCommunities();
    
    return res
      .status(200)
      .json(new ApiResponse(200, results, "Scraping completed for all communities"));
  } catch (error) {
 ...
 */
router.get("/scrape/:communityId", asyncHandler(async (req, res) => {
 ...
 */
router.post("/test/:platform", asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, testResults, `${platform} scraper test completed`));
  } catch (error) {
